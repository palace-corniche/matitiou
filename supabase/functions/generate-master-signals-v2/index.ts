...
Deno.serve(async (req) => {
  ...
  return new Response(JSON.stringify({
    ...
    if (tickError || !latestTick) {
      console.error('❌ No tick data available');
      throw new Error('No market data available for signal generation');
    }

    const currentPrice = (latestTick.bid + latestTick.ask) / 2;
    console.log('📊 Current EUR/USD price:', currentPrice);

    // Generate synthetic modular signals for demonstration
    const modularSignals = await generateModularSignals(supabase, currentPrice, latestTick);
    
    if (modularSignals.length === 0) {
      console.log('⚠️ No modular signals generated');
      return new Response(
        JSON.stringify({
          success: false,
          message: 'No modular signals available for fusion'
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log(`✅ Generated ${modularSignals.length} modular signals`);

    // Perform signal fusion
    const masterSignal = await fuseSignals(modularSignals, currentPrice, latestTick);
    
    if (!masterSignal) {
      console.log('⚠️ Signal fusion did not produce a tradeable signal');
      return new Response(
        JSON.stringify({
          success: false,
          message: 'Signal fusion criteria not met'
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Insert master signal into database
    ...
    if (insertError) {
      console.error('❌ Error inserting master signal:', insertError);
      throw insertError;
    }

    console.log('✅ Master signal created:', insertedSignal.id);

    // Update module performance tracking
    await updateModulePerformance(supabase, modularSignals);

    return new Response(
      JSON.stringify({
        success: true,
        master_signal: insertedSignal,
        modular_signals_count: modularSignals.length,
        timestamp: new Date().toISOString()
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    console.error('❌ Master signal generation error:', error);
    return new Response(
      JSON.stringify({
        success: false,
