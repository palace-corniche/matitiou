...
-- Update atomic_lock_signals to use 'pending_execution' instead of 'processing'
...
AS $$
BEGIN
  -- Atomically lock and update pending signals to 'pending_execution'
  -- SKIP LOCKED ensures no conflicts - each instance gets different signals
  RETURN QUERY
  UPDATE master_signals ms
  SET 
    status = 'pending_execution',  -- ✅ Use valid status value
