...
BEGIN
    ...
    peak_equity := portfolio_record.initial_deposit;
    max_drawdown_calc := 0;
    
    -- Simple max drawdown calculation
    FOR current_equity IN 
        SELECT 
            portfolio_record.initial_deposit + SUM(pnl) OVER (ORDER BY exit_time) as running_equity
        FROM shadow_trades 
        WHERE portfolio_id = NEW.portfolio_id AND status = 'closed'
        ORDER BY exit_time
    LOOP
        IF current_equity > peak_equity THEN
            peak_equity := current_equity;
        END IF;
        
        current_drawdown := ((peak_equity - current_equity) / peak_equity) * 100;
        IF current_drawdown > max_drawdown_calc THEN
            max_drawdown_calc := current_drawdown;
        END IF;
    END LOOP;
    ...
    SET 
        ...
        max_drawdown = max_drawdown_calc,
