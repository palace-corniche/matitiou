-- Phase 2: Database Schema & Data Contracts (Fixed)
-- Adding modular_signals, signal_audit, and master_signals tables with strict reproducibility

-- =============================================
-- Drop existing policies if they exist
-- =============================================
DROP POLICY IF EXISTS "Anyone can view modular signals" ON public.modular_signals;
DROP POLICY IF EXISTS "System can manage modular signals" ON public.modular_signals;
DROP POLICY IF EXISTS "Anyone can view signal audit" ON public.signal_audit;
DROP POLICY IF EXISTS "System can manage signal audit" ON public.signal_audit;
DROP POLICY IF EXISTS "Anyone can view master signals" ON public.master_signals;
DROP POLICY IF EXISTS "System can manage master signals" ON public.master_signals;

-- Drop existing tables if they exist
DROP TABLE IF EXISTS public.modular_signals CASCADE;
DROP TABLE IF EXISTS public.signal_audit CASCADE;
DROP TABLE IF EXISTS public.master_signals CASCADE;
...
-- =============================================
CREATE INDEX idx_modular_signals_analysis_id ON public.modular_signals(analysis_id);
CREATE INDEX idx_modular_signals_symbol_timeframe ON public.modular_signals(symbol, timeframe);
CREATE INDEX idx_modular_signals_timestamp ON public.modular_signals(timestamp DESC);
CREATE INDEX idx_modular_signals_active ON public.modular_signals(is_active) WHERE is_active = true;

CREATE INDEX idx_signal_audit_signal_id ON public.signal_audit(signal_id);
CREATE INDEX idx_signal_audit_analysis_id ON public.signal_audit(analysis_id);
CREATE INDEX idx_signal_audit_timestamp ON public.signal_audit(timestamp DESC);
CREATE INDEX idx_signal_audit_action_type ON public.signal_audit(action_type);

CREATE INDEX idx_master_signals_analysis_id ON public.master_signals(analysis_id);
CREATE INDEX idx_master_signals_symbol_timeframe ON public.master_signals(symbol, timeframe);
CREATE INDEX idx_master_signals_timestamp ON public.master_signals(timestamp DESC);
CREATE INDEX idx_master_signals_status ON public.master_signals(status);
CREATE INDEX idx_master_signals_signal_type ON public.master_signals(signal_type);
...
-- RLS Policies for modular_signals
CREATE POLICY "modular_signals_select_policy" 
ON public.modular_signals FOR SELECT 
USING (true);

CREATE POLICY "modular_signals_all_policy" 
ON public.modular_signals FOR ALL 
USING (true);

-- RLS Policies for signal_audit
CREATE POLICY "signal_audit_select_policy" 
ON public.signal_audit FOR SELECT 
USING (true);

CREATE POLICY "signal_audit_all_policy" 
ON public.signal_audit FOR ALL 
USING (true);

-- RLS Policies for master_signals
CREATE POLICY "master_signals_select_policy" 
ON public.master_signals FOR SELECT 
USING (true);

CREATE POLICY "master_signals_all_policy" 
...
-- =============================================
-- Validation function for reproducibility
-- =============================================
...
-- =============================================
-- Store migration info for rollback capability
-- =============================================
...
(
    ...
    jsonb_build_object(
        ...
        'can_rollback', true,
        'rollback_sql', 'DROP TABLE IF EXISTS public.modular_signals CASCADE; DROP TABLE IF EXISTS public.signal_audit CASCADE; DROP TABLE IF EXISTS public.master_signals CASCADE; DROP FUNCTION IF EXISTS public.create_signal_audit_log(); DROP FUNCTION IF EXISTS public.validate_signal_reproducibility(UUID, TEXT);'
    ),
    'Phase 2 database schema migration tracking for reversible deployment'
)
